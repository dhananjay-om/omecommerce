...
const PAGE_SELECT = { publicId: true, storeViewId: true, handle: true, title: true, body: true, status: true, publishedAt: true } as const;
...
export class PrismaCmsPageRepository implements CmsPageRepository {
  ...
  async findByPublicId(publicId: string): Promise<CmsPageRecord | null> {
    return this.db.cmsPage.findFirst({ where: { publicId }, select: PAGE_SELECT });
  }
  ...
  async existsByStoreViewAndHandle(storeViewId: bigint | null, handle: string): Promise<boolean> {
    const found = await this.db.cmsPage.findFirst({ where: { storeViewId, handle }, select: { id: true } });
    return found !== null;
  }

  async resolveForStoreView(storeViewId: bigint, handle: string): Promise<CmsPageRecord | null> {
    const specific = await this.db.cmsPage.findFirst({ where: { storeViewId, handle }, select: PAGE_SELECT });
    if (specific) return specific;
    return this.db.cmsPage.findFirst({ where: { storeViewId: null, handle }, select: PAGE_SELECT });
  }
}
