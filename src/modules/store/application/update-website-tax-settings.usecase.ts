...
/** GST Settings admin page — GST registration only (gstin/originStateCode/
 *  pricesIncludeTax); store branding lives on the separate General Settings
 *  page/UpdateWebsiteGeneralSettings, even though both write the same
 *  Website row — see WebsiteRepository's doc comment on why that split
 *  exists. Defense in depth alongside two DB CHECKs:
 *  website_gstin_state_code_paired (both fields must be set or cleared
 *  together, never half-configured) and website_gstin_state_match (the
 *  GSTIN's first 2 digits must equal originStateCode — a GSTIN is
 *  state-specific by construction). Without this pre-check, a mismatch here
 *  would hit the raw Postgres CHECK and surface as an unhandled 500 instead
