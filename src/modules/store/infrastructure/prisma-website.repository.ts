...
export class PrismaWebsiteRepository implements WebsiteRepository {
  ...
  async list(): Promise<WebsiteInfo[]> {
    const rows = await this.db.website.findMany({
      orderBy: { code: 'asc' },
      select: { publicId: true, code: true, name: true, gstin: true, originStateCode: true },
    });
    return rows;
  }

  async update(code: string, input: { gstin?: string | null; originStateCode?: string | null }): Promise<WebsiteInfo> {
    const row = await this.db.website.update({
      where: { code },
      data: { gstin: input.gstin, originStateCode: input.originStateCode },
      select: { publicId: true, code: true, name: true, gstin: true, originStateCode: true },
    });
    return row;
  }
