...
/** Same requireCustomer + ownership-checked shape as RemoveWalletFromCart. No-op if no credit-terms tender is applied. */
export class RemoveCreditTermsFromCart {
  constructor(
  ...
  async execute(cmd: { cartPublicId: string; customerPublicId: string }): Promise<CartView> {
    ...
    await this.carts.removeTender(cart.id, 'CREDIT_TERMS', null);
