-- Migration: b2b_credit_terms_checks (plan/15 Phase 7)
-- Pure raw-SQL migration — no Prisma schema changes of its own. Split out of
-- the prior migration (20260819130000_b2b_credit_terms) because Postgres
-- refuses to reference a just-ALTER-TYPE-ADD-VALUE'd enum label within the
-- same transaction that added it — see prisma/sql/0015_credit_terms_checks_raw.sql.
