...
import type { Company, CompanyMember, CompanyMemberRole, CompanyStatus } from '@/lib/types';
...
function readCompanyFields(formData: FormData) {
  ...
  return { name, customerGroupCode, taxExempt, taxExemptionRef, gstin, billingContactName, billingContactEmail, billingContactPhone };
}
...
export async function createCompany(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const { name, customerGroupCode, taxExempt, taxExemptionRef, gstin, billingContactName, billingContactEmail, billingContactPhone } =
  ...
  if (taxExempt && !taxExemptionRef) {
    return { error: 'A tax exemption reference is required when tax exempt is checked.', success: false };
  }
...
export async function setCompanyStatus(publicId: string, status: CompanyStatus, _prevState: ActionState): Promise<ActionState> {
  try {
...
export async function addCompanyMember(publicId: string, _prevState: ActionState, formData: FormData): Promise<ActionState> {
...
export async function updateCompanyMemberRole(
  ...
  try {
    await apiPut<CompanyMember>(`/admin/v1/companies/${publicId}/members/${customerPublicId}`, { role });
  } catch (err) {
    if (err instanceof ApiError) return { error: err.message, success: false };
    throw err;
  }
...
export async function removeCompanyMember(publicId: string, customerPublicId: string, _prevState: ActionState): Promise<ActionState> {
  try {
