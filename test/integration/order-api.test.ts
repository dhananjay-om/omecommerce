...
describe.skipIf(!process.env.INTEGRATION)('order API (live DB)', () => {
  ...
  beforeAll(async () => {
    ...
    await request(app).post('/admin/v1/warehouses').send({ code: 'ORD-WH', name: 'Order Warehouse' });
    await request(app)
      .post('/admin/v1/shipping-methods')
      .send({ code: 'STANDARD', name: 'Standard Shipping', flatRate: '5.00', currency: 'USD' });
    await request(app).post('/admin/v1/tax-classes').send({ code: 'STANDARD-TAX', name: 'Standard Tax', rate: '0.10' });
  });
  ...
  it('completes checkout end-to-end: cart -> reserve -> order -> paid, stock deducted', async () => {
    ...
    await request(app)
      ...
      .send({ variantId, warehouseCode: 'ORD-WH', delta: 10, reason: 'PURCHASE' });
    ...
    const stock = await request(app).get('/admin/v1/inventory/stock').query({ variantId, warehouseCode: 'ORD-WH' });
    expect(stock.body.data).toEqual({ onHand: 8, reserved: 0, available: 8 });

    // GET order by publicId works too
    const fetched = await request(app).get(`/admin/v1/orders/${order.publicId}`);
  ...
  it('applies tax when the product has a tax class assigned', async () => {
    ...
    await request(app)
      .post('/admin/v1/inventory/adjustments')
      .send({ variantId, warehouseCode: 'ORD-WH', delta: 5, reason: 'PURCHASE' });
  ...
  it('rejects checkout when inventory is insufficient, without side effects', async () => {
    ...
    await request(app)
      .post('/admin/v1/inventory/adjustments')
      .send({ variantId, warehouseCode: 'ORD-WH', delta: 2, reason: 'PURCHASE' });
  ...
  it('fulfills, then refunds with restock, updating statuses and stock', async () => {
    ...
    const overFulfill = await request(app)
      .post(`/admin/v1/orders/${orderPublicId}/fulfillments`)
  ...
  it('cancels an unfulfilled order: full refund + restock + status CANCELLED', async () => {
    ...
    const cancel = await request(app).post(`/admin/v1/orders/${orderPublicId}/cancel`);
