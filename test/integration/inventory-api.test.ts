...
describe.skipIf(!process.env.INTEGRATION)('inventory API (live DB)', () => {
  ...
  it('runs the full ledger lifecycle: adjust -> reserve -> commit -> reserve -> release', async () => {
    ...
    let stock = await request(app).get('/admin/v1/inventory/stock').query({ variantId, warehouseCode: 'WH-LIFECYCLE' });
    expect(stock.body.data).toEqual({ onHand: 10, reserved: 3, available: 7 });

    // commit -> on_hand -= 3, reserved -= 3
    const commit = await request(app).post(`/admin/v1/inventory/reservations/${r1.body.data.reservationId}/commit`);
    expect(commit.status).toBe(204);

    stock = await request(app).get('/admin/v1/inventory/stock').query({ variantId, warehouseCode: 'WH-LIFECYCLE' });
    expect(stock.body.data).toEqual({ onHand: 7, reserved: 0, available: 7 });

    // committing again fails (no longer ACTIVE)
    const recommit = await request(app).post(`/admin/v1/inventory/reservations/${r1.body.data.reservationId}/commit`);
    ...
    const release = await request(app).post(`/admin/v1/inventory/reservations/${r2.body.data.reservationId}/release`);
    ...
    const overCorrect = await request(app)
      .post('/admin/v1/inventory/adjustments')
  ...
  it('proves the guarded UPDATE is race-safe under concurrent HTTP reservations', async () => {
    await request(app).post('/admin/v1/warehouses').send({ code: 'WH-CONC', name: 'Concurrency Warehouse' });
    ...
    const stock = await request(app).get('/admin/v1/inventory/stock').query({ variantId, warehouseCode: 'WH-CONC' });
  ...
  it('sweeps expired reservations without touching still-active ones', async () => {
    await request(app).post('/admin/v1/warehouses').send({ code: 'WH-EXPIRY', name: 'Expiry Warehouse' });
    ...
    let stock = await request(app).get('/admin/v1/inventory/stock').query({ variantId, warehouseCode: 'WH-EXPIRY' });
    expect(stock.body.data).toEqual({ onHand: 5, reserved: 3, available: 2 });

    const sweep = await request(app).post('/admin/v1/inventory/reservations/sweep-expired');
    expect(sweep.status).toBe(200);
    expect(sweep.body.data.releasedCount).toBeGreaterThanOrEqual(1);

    stock = await request(app).get('/admin/v1/inventory/stock').query({ variantId, warehouseCode: 'WH-EXPIRY' });
  ...
  it('validates input and 404s on unknown warehouse/variant', async () => {
    ...
    const zeroDelta = await request(app)
      .post('/admin/v1/inventory/adjustments')
    ...
    const unknownVariant = await request(app)
